...
CMSIS_INLINE __STATIC_INLINE uint32_t __SMLAD(uint32_t x, uint32_t y,
                                              uint32_t sum) {
    return (
        (uint32_t) (((((q31_t) x << 16) >> 16) * (((q31_t) y << 16) >> 16)) +
...
CMSIS_INLINE __STATIC_INLINE uint64_t __SMLALD(uint32_t x, uint32_t y,
    ...
    return (
        (uint64_t) (((((q31_t) x << 16) >> 16) * (((q31_t) y << 16) >> 16)) +
                    ((((q31_t) x) >> 16) * (((q31_t) y) >> 16)) +
                    (((q63_t) sum))));
}
...
typedef struct {
    ...
    q15_t*
        pState; /**< points to the state variable array. The array is of length numStages. */
    q15_t*
...
typedef struct {
    uint16_t numTaps; /**< number of coefficients in the filter. */
    uint16_t
        stateIndex; /**< state buffer index.  Points to the oldest sample in the state buffer. */
    float32_t*
    ...
    int32_t*
        pTapDelay; /**< points to the array of delay values.  The array is of length numTaps. */
...
CMSIS_INLINE __STATIC_INLINE q15_t arm_pid_q15(arm_pid_instance_q15* S,
    ...
    acc =
        (q63_t) __SMLALD((uint32_t) S->A1, (uint32_t) *vstate, (uint64_t) acc);
#else
    /* acc = A0 * x[n]  */
    acc = ((q31_t) S->A0) * in;
...
/**
   ...
   */
CMSIS_INLINE __STATIC_INLINE void arm_clarke_q31(q31_t Ia, q31_t Ib,
                                                 q31_t* pIalpha,
                                                 q31_t* pIbeta) {
    q31_t product1,
...
/**
   ...
   * @param[in]  cosVal   cosine value of rotation angle theta
   */
CMSIS_INLINE __STATIC_INLINE void arm_inv_park_f32(float32_t Id, float32_t Iq,
                                                   float32_t* pIalpha,
                                                   float32_t* pIbeta,
                                                   float32_t sinVal,
                                                   float32_t cosVal) {
