# Tooling
There are a few things that anyone who is interested in working with this codebase should know. 
This is a short guide to setting up an environment to start using and developing this codebase as well as important system design choices which are strongly encouraged to be followed.
...
This will be the primary development interface for STM32 MCUs. It provides an integrated environment for project creation, code editing, building, flashing, and debugging all from within VSCode. The extension uses the same toolchain and device data packages used in STM32CubeIDE, but with the flexibility and modern tooling of VSCode. **Before installing this extension, ensure you have the prerequisites installed**. 
...
These are tools for making things related to development easier (e.g. test coverage, formatting, etc.). These are not strictly necessary for development and can be left out, HOWEVER, IT IS ENCOURAGED THESE TOOLS ARE SET UP due to their usefulness and their assistance in keeping things like styling consistent. 
