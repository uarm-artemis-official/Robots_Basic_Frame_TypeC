...
/*
    float v1x = chassis_hdlr->vx - chassis_hdlr->wz * (CHASSIS_WHEEL_X_LENGTH * 0.5) ;
    float v1y = chassis_hdlr->vy + chassis_hdlr->wz * (CHASSIS_WHEEL_Y_LENGTH * 0.5) ;
    float v2x = chassis_hdlr->vx + chassis_hdlr->wz * (CHASSIS_WHEEL_X_LENGTH * 0.5) ;
    float v2y = chassis_hdlr->vy + chassis_hdlr->wz * (CHASSIS_WHEEL_Y_LENGTH * 0.5) ;
    float v3x = chassis_hdlr->vx + chassis_hdlr->wz * (CHASSIS_WHEEL_X_LENGTH * 0.5) ;
    float v3y = chassis_hdlr->vy - chassis_hdlr->wz * (CHASSIS_WHEEL_Y_LENGTH * 0.5) ;
    float v4x = chassis_hdlr->vx - chassis_hdlr->wz * (CHASSIS_WHEEL_X_LENGTH * 0.5) ;
    float v4y = chassis_hdlr->vy - chassis_hdlr->wz * (CHASSIS_WHEEL_Y_LENGTH * 0.5) ;
...
/*
    ...
    chassis_hdlr->mec_spd[CHASSIS_WHEEL2_CAN_ID] =
        (int16_t) (sqrt(pow(v2x,2) + pow(v2y,2))) * CHASSIS_MOTOR_DEC_RATIO
    chassis_hdlr->mec_spd[CHASSIS_WHEEL3_CAN_ID] =
        (int16_t) (sqrt(pow(v3x,2) + pow(v3y,2))) * CHASSIS_MOTOR_DEC_RATIO;
...
//  (uint32_t) (atan2(v1y,v1x) * (1800/pi))
//if chassis_hdlr->swerve_angle[SWERVE_1_CAN_ID]
...
void chassis_update_gimbal_coord(Chassis_t* chassis_hdlr, int16_t* channels) {
    ...
    chassis_hdlr->gimbal_axis.vx = channels[2];  // apply vx data here
    chassis_hdlr->gimbal_axis.vy = channels[3];  // apply vy data here
...
void chassis_update_chassis_coord(Chassis_t* chassis_hdlr, int16_t* channels) {
    ...
    chassis_hdlr->vx = channels[2];  // apply vx data here
    chassis_hdlr->vy = channels[3];  // apply vy data here
