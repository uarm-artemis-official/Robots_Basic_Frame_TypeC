...
void Shoot_Task_Func(void const* argument) noexcept {
    /* Infinite loop */
    ...
    for (;;) {
        ...
        if (shoot.shoot_act_mode == SHOOT_CEASE) {
            ...
            set_mag_motor_angle(
                &shoot,
                shoot.mag_cur_angle + SHOOT_CONT_MAG_SPEED);  //keep spinning
            set_fric_motor_current(&shoot, LEVEL_ONE_CAN_SPD);
        }
...
void shoot_task_init(Shoot_t* sht, Motor_t* s_motors) {
    ...
    motor_init(&(s_motors[SHOOT_LOADER_2006_INDEX]), max_out_angle_mag_2006,
               max_I_out_angle_mag_2006, max_err_angle_mag_2006,  //angular loop
               kp_angle_mag_2006, ki_angle_mag_2006, kd_angle_mag_2006,
               max_out_spd_mag_2006, max_I_out_spd_mag_2006,
               max_err_spd_mag_2006,  //spd loop
               kp_spd_mag_2006, ki_spd_mag_2006, kd_spd_mag_2006,
               kf_spd_mag_2006);  //spd ff gain
    motor_init(&(s_motors[SHOOT_LEFT_FRIC_WHEEL_INDEX]), max_out_spd_fric,
               max_I_out_spd_fric, max_err_spd_fric, kp_spd_fric, ki_spd_fric,
               kd_spd_fric, 0, 0, 0, 0, 0, 0,  //no second loop
               0);                             //spd ff gain
    motor_init(&(s_motors[SHOOT_RIGHT_FRIC_WHEEL_INDEX]), max_out_spd_fric,
               max_I_out_spd_fric, max_err_spd_fric, kp_spd_fric, ki_spd_fric,
               kd_spd_fric, 0, 0, 0, 0, 0, 0,  //no second loop
               0);                             //spd ff gain
    ...
    memset(&(s_motors[SHOOT_LEFT_FRIC_WHEEL_INDEX].motor_feedback), 0,
           sizeof(Motor_Feedback_t));
    memset(&(s_motors[SHOOT_RIGHT_FRIC_WHEEL_INDEX].motor_feedback), 0,
           sizeof(Motor_Feedback_t));
    memset(&(s_motors[SHOOT_LOADER_2006_INDEX].motor_feedback), 0,
           sizeof(Motor_Feedback_t));
...
void shoot_fric_pwm_engagement(Shoot_t* sht, uint16_t target_pwm) {
    ...
    if (sht->fric_engage_flag == 0) {
        if (sht->fric_left_cur_spd >= target_pwm) {
            ...
            sht->fric_right_cur_spd =
                ramp_calculate(&shoot.fric_right_ramp) * target_pwm +
                target_pwm;
        ...
        sht->fric_left_cur_spd =
            ramp_calculate(&shoot.fric_left_ramp) * target_pwm + target_pwm;
...
/**
  ...
  * @retval    None
  */
void shoot_fric_can_engagement(Shoot_t* sht, Motor_t* s_motors,
                               uint16_t target_can) {
    /* obtain motor feedback for determining the current rpm */
    //	shoot_fric_get_feedback(sht);
    sht->fric_left_cur_spd =
        s_motors[SHOOT_LEFT_FRIC_WHEEL_INDEX].motor_feedback.rx_rpm;
    sht->fric_right_cur_spd =
        s_motors[SHOOT_RIGHT_FRIC_WHEEL_INDEX].motor_feedback.rx_rpm;
    ...
    shoot_calc_fric_pid_out(&(shoot_motors[SHOOT_LEFT_FRIC_WHEEL_INDEX]),
                            -sht->fric_can_tar_spd);
    shoot_calc_fric_pid_out(&(shoot_motors[SHOOT_RIGHT_FRIC_WHEEL_INDEX]),
                            sht->fric_can_tar_spd);
...
void shoot_execute(Shoot_t* sht, Motor_t* s_motors) {
    ...
    if (sht->shoot_act_mode == SHOOT_CEASE ||
        sht->shoot_act_mode == SHOOT_RESERVE) {
        ...
        if (sht->fric_engage_flag == 0 &&
            sht->fric_counter >= FRIC_CAN_RAMP_DELAY) {
            //			osDelay(500);
            sht->fric_engage_flag = 1;
            sht->fric_counter = 0;
        }
...
void shoot_get_motor_feedback(Shoot_t* shoot, Motor_t* s_motors) {
    ...
    BaseType_t motor_feedback_message =
        peek_message(MOTOR_READ, motor_feedbacks, 0);
    if (motor_feedback_message == pdTRUE) {
        memcpy(&(shoot->mag_fb), &motor_feedbacks[SHOOT_LOADER_CAN_ID],
               sizeof(Motor_Feedback_t));
        memcpy(&(s_motors[SHOOT_LEFT_FRIC_WHEEL_INDEX].motor_feedback),
               &motor_feedbacks[SHOOT_LEFT_FRIC_CAN_ID],
               sizeof(Motor_Feedback_t));
        memcpy(&(s_motors[SHOOT_RIGHT_FRIC_WHEEL_INDEX].motor_feedback),
               &motor_feedbacks[SHOOT_RIGHT_FRIC_CAN_ID],
               sizeof(Motor_Feedback_t));
        memcpy(&(s_motors[SHOOT_LOADER_2006_INDEX].motor_feedback),
               &motor_feedbacks[SHOOT_LOADER_CAN_ID], sizeof(Motor_Feedback_t));
    }
}
...
void shoot_mag_get_rel_angle(Shoot_t* sht) {
    ...
    sht->mag_cur_angle =
        (sht->mag_turns_counter * 2 * PI /
         SHOOT_MAG_GEAR_RATIO) +  // the angle the turns has been done
        (sht->mag_fb.rx_angle / 8192 * (2 * PI) /
         SHOOT_MAG_GEAR_RATIO);  // the current rx angle
 ...
 * */
int16_t shoot_mag_update_turns(Shoot_t* sht, int16_t raw_ecd,
                               int16_t prev_ecd) {
...
void shoot_calc_loader_pid_out(Shoot_t* sht, Motor_t* s_motors) {
    s_motors[SHOOT_LOADER_2006_INDEX].tx_data = (int32_t) pid_dual_loop_control(
        sht->mag_tar_angle,
        &(s_motors[SHOOT_LOADER_2006_INDEX].motor_info.f_pid),
        &(s_motors[SHOOT_LOADER_2006_INDEX].motor_info.s_pid),
        sht->mag_cur_angle, sht->mag_fb.rx_rpm, SHOOT_TASK_EXEC_TIME * 0.001);
}

void shoot_calc_fric_pid_out(Motor_t* motor, float target) {
    motor->tx_data = pid_single_loop_control(target, &(motor->motor_info.f_pid),
...
void shoot_send_motor_volts(Motor_t* s_motors) {
    ...
    motor_set_message.motor_can_volts[SHOOT_LOADER_CAN_ID] =
        s_motors[SHOOT_LOADER_2006_INDEX].tx_data;
    motor_set_message.motor_can_volts[SHOOT_LEFT_FRIC_CAN_ID] =
        s_motors[SHOOT_LEFT_FRIC_WHEEL_INDEX].tx_data;
    motor_set_message.motor_can_volts[SHOOT_RIGHT_FRIC_CAN_ID] =
        s_motors[SHOOT_RIGHT_FRIC_WHEEL_INDEX].tx_data;
...
void shoot_detect_mag_status(Shoot_t* sht) {
    if (sht->shoot_act_mode != SHOOT_CEASE) {
        ...
        if (shoot_check_flag != 1 ||
            (shoot_check_flag == 1 && shoot_check_counter < 10))
