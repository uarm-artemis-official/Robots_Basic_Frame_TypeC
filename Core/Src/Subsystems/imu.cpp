...
static void bmi088_cali_slove(float gyro[3], float accel[3],
...
static int ahrs_update(AhrsSensor_t* sensor) {
...
static void bmi088_cali_slove(float gyro[3], float accel[3],
    ...
    for (uint8_t i = 0; i < 3; i++) {
        gyro[i] = bmi088->gyro[0] * gyro_scale_factor[i][0] +
                  bmi088->gyro[1] * gyro_scale_factor[i][1] +
