...
float in_out_map(float input, float in_min, float in_max, float out_min,
    ...
    ASSERT(input >= in_min && input <= in_max,
           "input has to be in [in_min, in_max].");
...
float first_order_low_pass_filter(first_order_low_pass_t* folp, float data) {
    ...
    /* apply equation */
    folp->output_data =
        folp->a * folp->cur_data + (1.0 - folp->a) * folp->last_output_data;
...
float first_order_high_pass_filter(first_order_high_pass_t* fohp, float data) {
    ...
    /* apply equation */
    fohp->output_data = fohp->a * fohp->last_output_data +
                        (fohp->a) * (fohp->input_data - fohp->last_input_data);
...
float ewma_filter(ewma_filter_t* ewma, float cur_data) {
    ewma->output_data =
        ewma->a * cur_data + (1.0 - ewma->a) * ewma->last_output_data;
 ...
 */
float sliding_window_mean_filter(sliding_mean_filter_t* filter,
                                 float new_data) {
