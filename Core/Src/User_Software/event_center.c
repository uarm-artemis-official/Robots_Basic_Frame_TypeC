...
EventBits_t wait_events(EventBits_t wait_events, TickType_t timeout) {
	return xEventGroupWaitBits(events_group, wait_events, wait_events, pdTRUE, timeout);
}
...
BaseType_t sync_tasks(Sync_Event_t sync_event, EventBits_t set_task, TickType_t timeout) {
	...
	for (int i = 0; i < NUM_SYNC_GROUPS; i++) {
		if (sync_groups[i].sync_event == sync_event) active_group_index = i;
		if (sync_groups[i].sync_event == None) available_group_index = i;
	}

	int sync_group_index = active_group_index != -1 ? active_group_index : available_group_index;
	if (sync_group_index != -1) {
		sync_groups[sync_group_index].sync_event = sync_event;
		EventBits_t rv = xEventGroupSync(sync_groups[sync_group_index].event_group, set_task, (EventBits_t) sync_event, timeout);
