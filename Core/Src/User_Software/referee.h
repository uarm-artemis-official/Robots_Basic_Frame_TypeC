/*
 * Referee system application header
 *   Send and Recv the data from the referee system
 *
 * Created on Jul, 2023
 *		Author: Haoran
 *
...
typedef struct __attribute__((__packed__))//0x0201
{
	...
	uint16_t current_HP;
	uint16_t maximum_HP;
	uint16_t shooter_id1_17mm_barrel_cooling_value;//per second
	uint16_t shooter_id1_17mm_barrel_heat_limit;
	uint16_t shooter_id1_17mm_initial_launching_speed_limit;
	uint16_t shooter_id2_17mm_barrel_cooling_valuecooling_rate;//per second
