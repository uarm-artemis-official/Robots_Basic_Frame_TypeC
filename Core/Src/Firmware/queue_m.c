...
void enqueueCanMessage(CAN_TxHeaderTypeDef* header, CanMessage_t* canQueue,
    ...
    if (((qm->tail + 1) % qm->queue_size) == qm->head) {
    ...
    /* tail ++ */
    qm->tail = (qm->tail + 1) % qm->queue_size;
}
...
void sendNextCanMessage(CAN_HandleTypeDef* hcan, CanMessage_t* canQueue,
    ...
    HAL_StatusTypeDef status = HAL_CAN_AddTxMessage(
        hcan, &(canQueue[qm->head].header), canQueue[qm->head].data, &mailbox);
    if (status == HAL_OK) {
        /* Message has been added to the mailbox successfully, remove it from the queue */
        qm->head = (qm->head + 1) % qm->queue_size;
    }
}