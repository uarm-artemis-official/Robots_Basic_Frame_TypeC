...
void atti_math_calc(AhrsSensor_t* ahrs, Attitude_t* atti) {
    ...
    accel_pitch = atan2f(
        ahrs->ay, 1.0f / invSqrt(ahrs->ax * ahrs->ax + ahrs->az * ahrs->az));
    accel_roll = atan2f(
        -ahrs->ax, 1.0f / invSqrt(ahrs->ay * ahrs->ay + ahrs->az * ahrs->az));

    /* update the estimated value of the tilt angle */
    // note ahrs->wx * dt is acatually the integral of the wx (rate)
    atti->pitch = GYRO_WEIGHT * (atti->pitch + ahrs->wx * SAMPLE_DT) +
                  ACCEL_WEIGHT * accel_pitch;  //accel cali
    atti->roll = GYRO_WEIGHT * (atti->roll + ahrs->wy * SAMPLE_DT) +
                 ACCEL_WEIGHT * accel_roll;  //accel cali
