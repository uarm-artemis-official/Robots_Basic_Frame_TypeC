...
//The init of the quaternion is q = (cos(0.5*angle), sin(0.5*a)*vx, sin(0.5*a)*vy, sin(0.5*a)*vz)
//Since the angle is 0, thus (1,0,0,0)
//Specifically, the cos(0.5*angle) (q0) represents the actual rotated angle(real part)
...
void mahony_ahrs_update(AhrsSensor_t *sensor, Attitude_t *atti)
{
  ...
  if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)))
  {
    ...
    // Compute and apply integral feedback if enabled(pi controller)
    if (twoKi > 0.0f)
    {
