...
TEST_F(VarFIFOTest, FillIndicesAndCapacityLimits) {
    ...
    EXPECT_TRUE(fifo.push(v0));
    EXPECT_TRUE(fifo.push(v1));
    EXPECT_TRUE(fifo.push(v2));
    EXPECT_TRUE(fifo.push(v3));
...
TEST_F(VarFIFOTest, MixedTypesDifferentSizes) {
    ...
    EXPECT_TRUE(fifo.push(s1));
    EXPECT_TRUE(fifo.push(i1));
    EXPECT_TRUE(fifo.push(s));
    EXPECT_TRUE(fifo.push(s2));
    EXPECT_FALSE(fifo.push(s3));  // Should fail, no space.
    ...
    EXPECT_TRUE(fifo.pop(&out_s1));
    EXPECT_EQ(out_s1, s1);

    EXPECT_TRUE(fifo.pop(&out_i1));
    EXPECT_EQ(out_i1, i1);

    EXPECT_FALSE(fifo.push(s3));  // Still no space.

    EXPECT_TRUE(fifo.pop(&out_s));
    EXPECT_EQ(out_s.a, s.a);
    EXPECT_EQ(out_s.b, s.b);

    EXPECT_TRUE(fifo.push(s3));  // Now there should be space.

    EXPECT_TRUE(fifo.pop(&out_s2));
    EXPECT_EQ(out_s2, s2);

    EXPECT_TRUE(fifo.pop(&out_s3));
